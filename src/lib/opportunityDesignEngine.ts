...
export interface MorphologicalSearchResult {
  ...
  blockedVectors: BlockedVector[];
  /** Zwicky box rows — one per active dimension */
  zwickyBox: ZwickyBoxRow[];
}
...
export function runMorphologicalSearch(
  ...
  // Stage 8: Build Zwicky box from baseline + qualified/blocked vectors
  const zwickyBox = buildZwickyBox(baseline, qualifiedVectors, blockedVectors);
