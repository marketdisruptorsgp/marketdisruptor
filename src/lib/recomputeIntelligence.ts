...
import { buildDiagnosticContext, type DiagnosticMode } from "@/lib/diagnosticContext";
import {
  extractAllEvidence,
  flattenEvidence,
...
export async function recomputeIntelligenceAsync(input: IntelligenceInput): Promise<IntelligenceOutput> {
  ...
  const morphContext = buildDiagnosticContext(input.analysisType as DiagnosticMode, input.lensConfig as unknown as import("@/components/LensToggle").UserLens | null);
