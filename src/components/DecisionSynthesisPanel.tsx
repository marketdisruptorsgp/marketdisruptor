    ...
    >
      ...
      <div className="p-4 flex items-center gap-3" style={{ borderBottom: `1px solid hsl(var(--cin-depth-fg))` }}>
        ...
        <div className="flex-1">
          ...
          <p className="text-xs font-medium mt-0.5" style={{ color: "hsl(var(--cin-label) / 0.5)" }}>
        ...
        <div className="text-right">
          ...
          <span className="text-xs font-bold" style={{ color: "hsl(var(--cin-label) / 0.4)" }}>/100</span>
        </div>
      </div>
      ...
      <div className="px-4 pt-3 pb-2">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "hsl(var(--cin-label) / 0.4)" }}>
            Confidence
          </span>
        </div>
      ...
      {decisionSynthesis?.blocking_uncertainties && decisionSynthesis.blocking_uncertainties.length > 0 && (
        <div className="px-4 pb-3">
          <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "hsl(var(--cin-label) / 0.4)" }}>
            Blocking Uncertainties
          </p>
          <div className="space-y-1.5">
            {decisionSynthesis.blocking_uncertainties.map((u, i) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <span className="text-xs leading-snug" style={{ color: "hsl(0 0% 85%)" }}>{u}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {decisionSynthesis?.fastest_validation_experiment && (
        <div className="px-4 pb-3">
          <div className="flex items-start gap-2 p-2.5 rounded-lg" style={{ background: "hsl(var(--cin-depth-bg) / 0.5)" }}>
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: "hsl(229 89% 63%)" }}>
                Fastest Validation
              </p>
              <p className="text-xs leading-snug" style={{ color: "hsl(0 0% 85%)" }}>
                {decisionSynthesis.fastest_validation_experiment}
              </p>
            </div>
          </div>
        ...
        <Collapsible>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "hsl(var(--cin-label) / 0.5)" }}>
                Falsification Protocol
              </span>
              <span className="text-xs font-extrabold px-1.5 py-0.5 rounded" style={{ color: fragilityColor, background: `${fragilityColor}15` }}>
          ...
          <CollapsibleContent>
            <div className="px-4 pb-4 space-y-3">
              ...
              {falsification.falsification_conditions.length > 0 && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1.5" style={{ color: "hsl(var(--cin-red) / 0.7)" }}>
                  ...
                  {falsification.falsification_conditions.map((c, i) => (
                    <div key={i} className="flex items-start gap-2 mb-1">
                      ...
                      <span className="text-xs leading-snug" style={{ color: "hsl(0 0% 80%)" }}>{c}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {falsification.economic_collapse_scenario && (
                <div className="p-2.5 rounded-lg" style={{ background: "hsl(var(--cin-red) / 0.05)", border: "1px solid hsl(var(--cin-red) / 0.1)" }}>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: "hsl(var(--cin-red) / 0.6)" }}>
                    Economic Collapse Scenario
                  </p>
                  <p className="text-xs leading-snug" style={{ color: "hsl(0 0% 80%)" }}>
                    {falsification.economic_collapse_scenario}
                  </p>
                </div>
              )}
              ...
              {falsification.adoption_failure_conditions.length > 0 && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1.5" style={{ color: "hsl(38 92% 50% / 0.7)" }}>
