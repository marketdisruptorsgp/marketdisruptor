 ...
 * Uses actual document-specific language instead of generic templates.
 */
...
export const SoWhatHeader = memo(function SoWhatHeader({
  ...
  const inactionRisk = narrative?.verdictRationale
    ? trimAt(narrative.verdictRationale, 150)
    : constraint
      ? trimAt(constraint, 150)
      : narrative?.trappedValue
        ? trimAt(humanizeLabel(narrative.trappedValue), 150)
        : null;

  // Build projected outcome — specific to the thesis
  const actionOutcome = thesis?.economicMechanism?.valueCreation
    ? trimAt(thesis.economicMechanism.valueCreation, 150)
    : thesis?.summary
      ? trimAt(thesis.summary, 150)
    ...
    >
      {inactionRisk && (
        <div className="flex items-start gap-2 flex-1 min-w-0">
          ...
          <p className="text-sm text-foreground/80 leading-snug">
            <span className="font-bold text-destructive">Risk:</span>{" "}
            {inactionRisk}
          </p>
        </div>
      )}
      ...
      {actionOutcome && (
        <div className="flex items-start gap-2 flex-1 min-w-0">
          ...
          <p className="text-sm text-foreground/80 leading-snug">
            <span className="font-bold" style={{ color: modeAccent }}>Opportunity:</span>{" "}
            {actionOutcome}
          </p>
        </div>
      )}
