...
export const BundleDeepDive = ({ opportunity, businessContext, index }: BundleDeepDiveProps) => {
  ...
  const handleExpand = async () => {
    ...
    try {
      ...
      if (error || !result?.success) { toast.error("Could not generate deep dive"); console.error(error || result?.error); return; }
      setData(result.data);
    } catch (err) { toast.error("Deep dive failed"); console.error(err); } finally { setIsLoading(false); }
  };
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      if (error || !result?.success) { toast.error("Regeneration failed"); return; }
      setData(result.data);
      toast.success("Deep dive refreshed!");
    } catch { toast.error("Regeneration failed"); } finally { setIsLoading(false); }
  };

  return (
    <div className="rounded-xl overflow-hidden transition-all" style={{ background: "hsl(var(--muted))", border: `1px solid hsl(var(--border))` }}>
      <button onClick={handleExpand} className="w-full flex items-start gap-2 p-3 text-left transition-colors hover:bg-primary/5">
        <div className="mt-0.5 flex-shrink-0 transition-transform" style={{ color: "hsl(var(--primary))" }}>
          {isOpen ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
        </div>
        <div className="flex-1">
          <span className="typo-card-body text-foreground/80 leading-relaxed">{opportunity}</span>
        </div>
      </button>
        ...
        <div className="px-3 pb-4 space-y-3">
          {isLoading ? (
            <div className="flex items-center gap-2 justify-center py-6">
              ...
              <span className="typo-card-body font-medium" style={{ color: "hsl(var(--primary))" }}>Generating deep dive…</span>
            </div>
          ) : data ? (
            <>
              <div className="flex justify-end">
                <button onClick={handleRegenerate}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-lg typo-card-meta font-semibold transition-all hover:scale-105"
                  style={{ background: "hsl(var(--primary) / 0.12)", color: "hsl(var(--primary))" }}>
                  <RefreshCw size={9} /> Regenerate
                </button>
              </div>

              <div className="p-3 rounded-lg typo-card-body leading-relaxed" style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))" }}>
              ...
              <div className="space-y-1.5">
                <p className="typo-status-label text-muted-foreground flex items-center gap-1">
                  <Rocket size={10} /> Implementation Steps
                </p>
                {data.implementationSteps.map((step, i) => (
                  <div key={i} className="flex items-start gap-2 typo-card-body p-2 rounded-lg" style={{ background: "hsl(var(--muted))" }}>
                    <span className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center typo-card-meta font-bold"
              ...
              {data.risks?.length > 0 && (
                <div className="space-y-1.5">
                  ...
                  {data.risks.map((risk, i) => (
                    <div key={i} className="flex items-start gap-2 p-2 rounded-lg typo-card-body" style={{ background: "hsl(var(--muted))" }}>
...
const InfoCard = ({ icon, label, text }: { icon: React.ReactNode; label: string; text: string }) => (
  <div className="p-2.5 rounded-lg" style={{ background: "hsl(var(--muted))" }}>
    <p className="typo-status-label text-muted-foreground flex items-center gap-1 mb-1">
      {icon} {label}
    </p>
    <p className="typo-card-body text-foreground/80 leading-relaxed">{text}</p>
  </div>
);
