...
import {
  ...
  CheckCircle2, ChevronDown, X, Sparkles, ArrowRight, Maximize2,
} from "lucide-react";
...
function MapLegend() {
  return (
    <div className="absolute top-4 right-4 z-20 flex items-center gap-4 px-3.5 py-2 rounded-xl text-[10px] font-bold"
      ...
      >
        ...
        {!isActive ? (
          ...
          <div
            ...
            style={{ background: STATUS.active.bg, color: STATUS.active.text, border: `1.5px solid ${STATUS.active.border}` }}
          >
...
function MobileTree({
  ...
  return (
    <div className="space-y-2.5">
      {/* Mobile legend */}
      <div className="flex items-center gap-4 px-3 py-2 rounded-xl text-[10px] font-bold"
        style={{ background: "hsl(var(--muted) / 0.3)", border: "1px solid hsl(var(--border) / 0.3)" }}
      >
        <span className="text-muted-foreground uppercase tracking-wider">Key</span>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full" style={{ background: STATUS.active.badge }} />
          <span style={{ color: STATUS.active.text }}>Active</span>
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full border" style={{ borderColor: "hsl(var(--border))", background: "hsl(var(--card))" }} />
          <span className="text-muted-foreground">Alternative</span>
        </span>
      </div>
                ...
                <div className="pt-1.5">
                  {!isActive ? (
                    ...
                    <p className="text-[10px] text-center font-bold py-1.5" style={{ color: STATUS.active.text }}>
                      Active — guiding downstream analysis
                    </p>
                  )}
                </div>
              </div>
