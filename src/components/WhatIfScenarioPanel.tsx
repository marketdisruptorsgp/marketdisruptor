...
function projectConstraintRemoval(
  ...
  const originalHot = getDimensionsByStatus(
    identifyActiveDimensions(baseline, allConstraints, leveragePoints),
    "hot",
  );
  ...
  const originalHotIds = new Set(originalHot.map(d => d.id));
  const originalWarmIds = new Set(
    getDimensionsByStatus(
      identifyActiveDimensions(baseline, allConstraints, leveragePoints),
      "warm",
    ).map(d => d.id),
  );
  const originalActiveIds = new Set([...originalHotIds, ...originalWarmIds]);
