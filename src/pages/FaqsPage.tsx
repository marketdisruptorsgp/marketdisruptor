...
const FAQS = [
  ...
  { q: "How do the different analysis modes work?", a: "Each mode — Product, Service, and Business Model — runs its own tailored pipeline through the same 5-step journey (Selection → Intelligence Report → Disrupt → Stress Test → Pitch Deck). Product mode focuses on physical goods: sourcing costs, supply chain mapping, patent landscape, and community demand signals. In Service Mode, the focus shifts from product details to analyzing customer journey friction, operational workflows, and technology use. In Business Model Mode, the process expands into a full strategic teardown covering revenue models, cost structures, competitive advantages, and potential disruption risks. Each mode produces its own customized Intelligence Report, adversarial Stress Test, and investor-ready Pitch Deck." },
