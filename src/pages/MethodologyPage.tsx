    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-border bg-card">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-7 sm:py-9">
          <p className="text-sm font-bold uppercase tracking-widest text-primary mb-2">Methodology</p>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-10">
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Every analysis runs through this sequential protocol. No step is skipped.</p>
          <div className="border border-border rounded-md bg-card p-3 sm:p-4">
            <ol className="space-y-0">
              {ENGINE_STEPS.map((s, i) => (
                <li key={i} className="flex items-center gap-3 py-1.5 text-sm border-b border-border/50 last:border-b-0">
                  <span className={`flex-shrink-0 w-5 h-5 rounded ${s.color} text-white flex items-center justify-center text-sm font-bold`}>
                    {i + 1}
                  </span>
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Established methodologies integrated into the reasoning engine.</p>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                        ...
                        <span
                          className="text-sm font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full"
                      ...
                      <p className="text-sm text-muted-foreground leading-relaxed">{fw.how}</p>
                    </div>
                  </div>
                </div>
              );
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Each mode applies frameworks through a different leverage domain.</p>
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">The 4-stage execution layer that operationalizes the reasoning engine.</p>
          <div className="space-y-1.5">
                ...
                <div key={i} className={`border border-border border-l-[3px] ${step.accent} rounded-md p-3 sm:p-3.5 bg-card`}>
                  <div className="flex items-start gap-2.5">
                    <div className="flex-shrink-0 flex flex-col items-center gap-0.5">
                      ...
                      <span className="text-sm font-bold text-muted-foreground">S{i + 1}</span>
                    </div>
                    <div>
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Context-aware data enrichment layers that activate based on category and geography.</p>
          <div className="space-y-1.5">
            <div className="border border-border border-l-[3px] border-l-cyan-500 rounded-md p-3 sm:p-3.5 bg-card">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="text-sm font-bold text-foreground mb-0.5">Geographic & Demographic Enrichment</p>
            ...
            <div className="border border-border border-l-[3px] border-l-rose-500 rounded-md p-3 sm:p-3.5 bg-card">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="text-sm font-bold text-foreground mb-0.5">Regulatory & Legal Context</p>
                  <p className="text-sm text-muted-foreground leading-relaxed mb-2">
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Lenses control <em>how</em> results are interpreted — same data, different strategic perspective.</p>
          <div className="space-y-1.5">
            {[
              {
                name: "Default",
              ...
              <div key={lens.name} className={`border border-border border-l-[3px] ${lens.accent} rounded-md p-3 sm:p-3.5 bg-card`}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full" style={{ background: `${lens.color}15`, color: lens.color }}>{lens.name}</span>
                </div>
                <p className="text-sm font-semibold text-foreground mb-0.5">{lens.desc}</p>
                <p className="text-sm text-muted-foreground leading-relaxed">{lens.details}</p>
              </div>
            ))}
          </div>
          <div className="mt-2 p-2.5 rounded-md bg-muted/50 border border-border/50">
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Purpose-built for Entrepreneurs Through Acquisition — evaluating, buying, and growing small businesses ($500K–$5M).</p>

          {/* Buyer Journey */}
          <div className="border border-border border-l-[3px] border-l-emerald-500 rounded-md bg-card p-3 sm:p-3.5 mb-2" >
            <p className="text-sm font-bold uppercase tracking-wider text-emerald-700 mb-2">The ETA Buyer Journey</p>
            <div className="grid grid-cols-5 gap-1 text-center">
              {[
                ...
                <div key={i} className="p-1.5 rounded bg-emerald-50 border border-emerald-200/50">
                  <p className="text-sm font-bold uppercase tracking-wider text-emerald-700">{p.phase}</p>
                  <p className="text-sm text-muted-foreground mt-0.5 leading-tight">{p.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            {[
              ...
              <div key={mod.title} className={`border border-border border-l-[3px] ${mod.accent} rounded-md p-3 sm:p-3.5 bg-card`}>
                <p className="text-sm font-bold text-foreground mb-1.5">{mod.title}</p>
                <ul className="space-y-0.5">
                  {mod.points.map((p, j) => (
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Profile-aware dominance modeling that re-ranks hypotheses based on your strategic archetype.</p>
          <div className="space-y-1.5">
            {[
              ...
              <div key={arch.name} className={`border border-border border-l-[3px] ${arch.accent} rounded-md p-3 sm:p-3.5 bg-card`}>
                <div className="flex items-center gap-2 mb-0.5">
                  <span className="text-sm font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full" style={{ background: `${arch.color}15`, color: arch.color }}>{arch.name}</span>
                </div>
                <p className="text-sm font-semibold text-foreground mb-0.5">{arch.desc}</p>
                <p className="text-sm text-muted-foreground leading-relaxed">{arch.details}</p>
              </div>
            ))}
          </div>
          <div className="mt-2 p-2.5 rounded-md bg-muted/50 border border-border/50">
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-3">Every analysis includes an interactive panel to question, challenge, and stress-test the reasoning.</p>
      ...
      <footer className="max-w-5xl mx-auto px-4 sm:px-6 py-4 border-t border-border text-center" />
    </div>
  );
}
